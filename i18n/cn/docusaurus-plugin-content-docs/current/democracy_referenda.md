...
下表列举了各信念系数及其对应的令牌锁定天数。 您可在 HDX 不锁定的情况下进行投票，但其权重只是很小的一部分（信念系数为 0.1）。 如下表所示，令牌锁定最多可达 192 天，信念系数也将提高到 6。
...
Alice 以 5000 HDX 和 0 天锁定期进行投票。
Bob 用 100 个 HDX 和 192 天锁定期投票。
